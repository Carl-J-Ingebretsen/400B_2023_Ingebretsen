...
def hernquist_mass(r,a=60*u.kpc,M_halo=1.975):
    ...
    Inputs: 
    r: astropy quantity of galacto centric distance in kpc
    a: scale radius of the profile in astropy kpc
    M_halo: mass of the halo in units of 1e12 Msun (float)
    Returns:
    Mass: astropy total mass within the given radius in Msun
    '''
...
# 
mdisk=7.5e10*u.Msun #From HW 3 disk mass of MW
mbulge=1e10*u.Msun#From HW 3 bulge mass of MW
...
# 
# R_j = r*(Msat/(2Mhost))**1/3
#Now get Msat = (Rj/r)**(3)*2*Mhost
def jacobi_mass(rj, r, M_host):
    '''This funciton determines the minimum satellite mass needed to maintain the size of the 
    satellite galaxy using the jacobi radius calcualtion for an isothermal sphere.
    Inputs:
    rj: the jacobi radius (astropy quantity kpc) or the observed stellar radius in kpc
    r: distance of the satellite from the host in astropy kpc
    M_host: mass of the host galaxy in astorpy Msun (within r)
    Returns:
    m_min: The minimum mass of the satellite galaxy in Msun
    '''
    m_min = (rj/r)**3*2*M_host #satellite mass
    return m_min
...
size_LMC = 18.5*u.kpc #observed radius of lmc disk (Mackey 2016)
distance_LMC = 50*u.kpc #distance to LMC
LMC_jacobiM = jacobi_mass(size_LMC,distance_LMC,mass_MW50)
...
#We want msat=(rtide/r)**3*4*mhost
def tidal_mass(r_tide, r, m_host):
    '''This function calculates the tidal mass of the galaxy
    inputs:
    r_tide: the observed size of satellite galaxy in kpc
    r: distance to the satellite in kpc
    M_host: mass of the host galaxy in astorpy Msun (within r)
    Returns:
    sat_mass: The minimum tidal mass of the satellite galaxy in Msun'''
    sat_mass = (r_tide/r)**3*4*m_host
    return sat_mass

LMC_tidalM = tidal_mass(size_LMC,distance_LMC,mass_MW50)
